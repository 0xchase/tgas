...
impl PythonTGA {
    ...
    pub fn train_with_python(
        ...
        let model_info = result.get("model_path")
            .and_then(|path| Some(json!({
    ...
    pub fn generate_with_python(
        ...
        let addresses = result.get("addresses")
...
impl TGA for PythonTGA {
    ...
    fn generate(&self) -> [u8; 16] {
        ...
        let kwargs = serde_json::json!({});
        let addresses = self.generate_with_python(1, false, kwargs)
...
pub fn get_available_python_tga_infos() -> Result<Vec<PythonTgaInfo>, String> {
    PYTHON_TGAS_INIT.call_once(|| {
        match query_python_tgas() {
            Ok(tgas) => {
...
mod tests {
    ...
    fn test_python_tga_discovery() {
        ...
        match result {
            ...
            Err(e) => {
                println!("Python TGA discovery failed (expected if Python not available): {}", e);
            }
        }
    }
} 